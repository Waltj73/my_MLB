...
def get_mlb_data():
    try:
        # Pulls current season standings data
...
if stats_df.empty:
    st.warning("⚠️ Official API currently unreachable. Manual Entry Mode Enabled.")
    ...
    st.success("Live MLB Stats Loaded via Official API")
...
for i in range(num_games):
    with st.expander(f"Game {i+1}", expanded=True):
        ...
        # Check if we have API data for the specific team name
        team_stats_available = not stats_df.empty and a_team in stats_df['Team'].values and h_team in stats_df['Team'].values

        if not team_stats_available:
            sc1, sc2, sc3, sc4 = st.columns(4)
            a_rpg = sc1.number_input(f"{a_team} R/G", value=4.50, step=0.01, format="%.2f", key=f"ar_{i}")
            a_ra = sc2.number_input(f"{a_team} RA/G", value=4.50, step=0.01, format="%.2f", key=f"ara_{i}")
            h_rpg = sc3.number_input(f"{h_team} R/G", value=4.50, step=0.01, format="%.2f", key=f"hr_{i}")
            h_ra = sc4.number_input(f"{h_team} RA/G", value=4.50, step=0.01, format="%.2f", key=f"hra_{i}")
        else:
